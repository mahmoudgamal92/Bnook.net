...
<div class="FilterTable">
        ...
        <?php
         $data = @file_get_contents("http://173.212.230.130/site/api/v2/ahmedhaies/data_from_yahoo?name=".$symbol."&is_new");
               if($data == false)
               {
...
?>
  ...
  <?php
            ...
            $interest = 0;
            if(isset($json['json_data']['quarterlyNetNonOperatingInterestIncomeExpense']) && 
            is_numeric($json['json_data']['quarterlyNetNonOperatingInterestIncomeExpense']) && 
            $json['json_data']['quarterlyNetNonOperatingInterestIncomeExpense'] > 0 )
            {
                $interest = $json['json_data']['quarterlyNetNonOperatingInterestIncomeExpense'];
            }
            
            $revenue =$json['json_data']['quarterlyTotalRevenue'];
            //Second Equation (less than 33 %)
            $Total_Assests = $json['json_data']['quarterlyTotalAssets'];  
            $debts  =$json['json_data']['quarterlyTotalDebt'];
            $region = $json['json_data']['region'];

            $check_industry =preg_match('
            (banks| alcoholic|casino|tobacco|rental|credit|capital|markets|insurance|motion|media|investment|media|rental|recreation|motels|asset|capitalmarkets|banking|marijuana|cannabis|brewers|lodging|wineries|distilleries|banks--diversified|capital markets|diversified)', $industry);
            
            $check_description = preg_match('(mariguana|cannabis|gamble|casino|wine|beer|vodka|scotch|hamburgers|hamburger|pig|pork|bacon|netflix|disney|chipotle|chuy|denny|darden|fatburger|taco|burger|mcDonald|loans|dolphin|animated| alcoholic|cruises)', $description);
    ...
    if ($industry == null  || ($revenue == 0 && $interest !=0))
    {
